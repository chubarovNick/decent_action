describe DecentAction::Controller::Wrapper do
  ...
  before do
    DecentAction.configure do |config|
      ...
      config.handle_exception(DecentAction::Exception::InvalidError, Proc.new { validation_error_handler })
    end
  end
  ...
  before do
    expect_any_instance_of(DecentAction::ActionWrappers::Exception).to receive(:wrap).and_call_original
    expect_any_instance_of(DecentAction::ActionWrappers::Validation).to receive(:wrap).and_call_original
  end
